...
class Mimic(nn.Module) : 
    def __init__(self, in_feature, out_feature, hidden_feature, hidden_layer) :
        ...
        self.mu = torch.zeros(out_feature)
        self.sig = torch.ones(out_feature)

    def set_reparm(self, mu, sig) : 
        self.mu, self.sig = mu, sig
        
    def forward(self, x, reparm=True) :
        ...
        for each_dim in range(x.size(1)) :
            ...
            if reparm : 
                reparm_predict_each_dim = predict_each_dim * self.sig[each_dim] + self.mu[each_dim]
            else : 
                reparm_predict_each_dim = predict_each_dim
                
            predict_list.append(reparm_predict_each_dim)
