...
def get_cifar1_dataset(batch_size, image_shape=(3,16,16)) : 
...
def gen_images(decoder, imagesize, z_prior) : 
    image=decoder(z_prior)
    return image.detach().cpu().view(-1, *imagesize)
...
image_shape = [3, 16, 16]
train_loader = get_cifar1_dataset(batch_size, image_shape)
latent_dim = 10

import hashlib
real_images_info_file_name = hashlib.md5(str(train_loader.dataset).encode()).hexdigest()+'.pickle'
infopickle_path = '../../inception_model_info/' + real_images_info_file_name
if os.path.exists(infopickle_path) : 
    print("Using saved info from ", infopickle_path)
    inception_model_score.load_real_images_info(infopickle_path)
else : 
    ...
    inception_model_score.save_real_images_info(infopickle_path)
...
import wandb
wandb.init(project='mlvu_aae')

fixed_noise = torch.rand(batch_size, latent_dim).to('cuda')
for epoch in range(0, epochs):
    for each_batch in tqdm.tqdm(train_loader):
        ...
        if epoch % 10 == 0 :
            decoder.eval()
            noise = torch.rand(batch_size, latent_dim).to('cuda')
            fake = gen_images(decoder, image_shape, noise)
    ...
    if epoch % 10 == 0 : 
        ...
        fake = gen_images(decoder, image_shape, fixed_noise)
        fake_np = vutils.make_grid(fake, nrow=8).permute(1,2,0).numpy()

        print("\t\tFID : %.4f, IS_f : %.4f, P : %.4f, R : %.4f, D : %.4f, C : %.4f" 
              %(fid, inception_score_fake, precision, recall, density, coverage))

        wandb.log({
            "epoch" : epoch,
            "fid" : fid,
            'fake_is':inception_score_fake,
            "precision":precision,
            "recall":recall,
            "density":density,
            "coverage":coverage,
