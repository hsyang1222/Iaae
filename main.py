...
import generative_model_score_with_hidden_representation
import torchvision
...
def main(args):
    ...
    customize
    '''
    if model_name in ['vanilla', 'yeop_n_iter', 'yeop_loss', 'n_han_iaae']:
    ...
    #####              Score              #####
    ###########################################
    if model_name == 'n_han_iaae':
        inception_model_score = generative_model_score_with_hidden_representation.GenerativeModelScore()
    else:
    ...
    for i in range(0, epochs):
        ...
        for each_batch in tqdm.tqdm(train_loader):
            ...
            '''
            if model_name == 'n_han_iaae':
                r_loss = update_autoencoder_with_feature(ae_optimizer, X_train_batch, encoder, decoder, inception_model_score, i, batch_count)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='FFHQ', choices=['LSUN_churches', 'LSUN_bedroom',
                                                                        'LSUN_cars', 'LSUN_cats',
                                                                        'FFHQ', 'CelebA', 'cifar10'])

    parser.add_argument('--model_name', type=str, default='', choices=['vanilla', 'yeop_loss',
                                                                       'yeop_n_iter', 'n_han_iaae'])
